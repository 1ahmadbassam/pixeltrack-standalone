...
fn setHitsLayerStart(   hitsModuleStart : UnsafePointer[UInt32],
...
struct PixelRecHitGPUKernel(Defaultable, Typeable):
    fn makeHits(self,
        ...
        if digis_d.nModules():  # protect from empty events
            getHits(cpeParams, UnsafePointer(to = bs_d), digis_d.view(), digis_d.nDigis(), clusters_d.view(), hits_d.view())
        ...
        if nHits:
            fillManyFromVector(hits_d.phiBinner()[], 10, hits_d.iphi(), hits_d.hitsLayerStart(), nHits)
