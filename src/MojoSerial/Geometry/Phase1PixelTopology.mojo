...
struct Phase1PixelTopology:
    ...
    alias numPixsInModule: UInt32 = Self.numRowsInModule.cast[DType.uint32]() * Self.numColsInModule.cast[DType.uint32]()
    ...
    @staticmethod
    fn _map_to_array[I: DType, R: Copyable & Movable, N: Int, func: fn[SIMD[I, 1]]() -> R]() -> List[R]:
        var arr: List[R] = []
        @parameter
        for i in range(N):
            arr.append(func[i]())
    ...
    alias layer: List[UInt8] = Self._map_to_array[DType.uint32, UInt8, Int(Self.layerIndexSize), Self.findLayerFromCompact]()
...
@fieldwise_init
struct AverageGeometry(Movable, Copyable):
